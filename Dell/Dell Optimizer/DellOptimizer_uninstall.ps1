...
##################################
# Remover Dell Optimizer APPX    #
##################################

# Verificar se o Dell Optimizer está instalado como APPX
$AppxPackageName = "DellOptimizer" # Nome do pacote APPX
$AppxPackage = Get-AppxPackage -AllUsers | Where-Object { $_.Name -like "*$AppxPackageName*" }

if ($null -ne $AppxPackage) {
    Write-Host "Dell Optimizer encontrado como APPX. Iniciando desinstalação..." -ForegroundColor Yellow

    try {
        # Remover o pacote APPX
        Remove-AppxPackage -Package $AppxPackage.PackageFullName -AllUsers -ErrorAction Stop
        Write-Host "Dell Optimizer (APPX) desinstalado com sucesso." -ForegroundColor Green
    } catch {
        Write-Host "Falha ao desinstalar o Dell Optimizer (APPX): $_" -ForegroundColor Red
    }
} else {
    Write-Host "Dell Optimizer não encontrado como APPX." -ForegroundColor Cyan
}